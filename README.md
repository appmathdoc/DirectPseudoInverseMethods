...
* __The CK Decomposition:__ If $B$ is upper bidiagonal, then there exists upper _invertible_ bidiagonal $C$ and a superdiagonal matrix $K$ (only non-zero coefficients are on the superdiagonal) such that
 math B = U \begin{bmatrix} C & 0 \\ 0 & K \end{bmatrix}
